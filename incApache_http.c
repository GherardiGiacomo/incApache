...
/*** TO BE DONE 6.0 START ***/
    ...
    if(pthread_mutex_lock(&cookie_mutex) != 0) 
		fail_errno("Errore locking mutex"); 

    int fl=1;
    while (fl){
        retval = ++CurUID;
        if(!UserTracker[retval]){        
            UserTracker[retval] = 0;
            fl=0;
        }
    }

    if(pthread_mutex_unlock(&cookie_mutex) != 0) fail_errno("Errore unlocking mutex"); 
...
/*** TO BE DONE 6.0 START ***/
    ...
    if(pthread_mutex_lock(&cookie_mutex) != 0) fail_errno("Error: mutex unlocking failed");

    ++UserTracker[myUID];
    newcount += UserTracker[myUID];

    if(pthread_mutex_unlock(&cookie_mutex) != 0) fail_errno("Error: mutex unlocking failed"); 
...
/*** TO BE DONE 6.0 START ***/
            ...
            if(stat(HTML_404, stat_p)==-1)			
                fail_errno("Errore in funzione stat");
            file_size=stat_p->st_size;
            file_modification_time=stat_p->st_mtim.tv_sec;
...
/*** TO BE DONE 6.0 START ***/
	...
	sprintf(http_header + strlen(http_header), "\r\nset cookie:%d, max age: %d", cookie, my_timegm(&time));
...
/*** TO BE DONE 6.0 START ***/
        ...
        if(sendfile(client_fd, fd, NULL, file_size) == -1)
            fail_errno("Errore sendfile");
        if(close(fd) == -1)	
            fail_errno("Errore close fd");
...
/*** TO BE DONE 6.0 START ***/
                ...
                if(my_timegm(&since_tm) >= stat_p->st_mtim.tv_sec)		
