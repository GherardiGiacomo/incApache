...
/*** TO BE DONE 6.1 START ***/
    ...
    i=MAX_THREADS-1;
	while(i>MAX_CONNECTIONS-1 && connection_no[i]!=conn_no) --i;
...
/*** TO BE DONE 6.0 START ***/
    ...
    if(pthread_mutex_unlock(&mime_mutex)) 
		fail_errno("Errore unlocking mutex: ");
...
/*** TO BE DONE 6.1 START ***/
    ...
    int count = new_thread_idx-1;
	while( (count>=MAX_CONNECTIONS) && (connection_no[count]!=connection_no[new_thread_idx])) count--;

	if(count==MAX_CONNECTIONS-1) to_join[new_thread_idx] = NULL;
	else 			    
		to_join[new_thread_idx] = &thread_ids[count];
